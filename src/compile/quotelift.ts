...
function gen_quote_lift(defuse: DefUseTable, scopes: Scope[], externs: string[]): Gen<QuoteLift> {
  return function(fself: QuoteLift): QuoteLift {
...
function quote_lift(tree: SyntaxNode, defuse: DefUseTable, scopes: Scope[], externs: string[]): Prog[] {
  let _quote_lift = fix(gen_quote_lift(defuse, scopes, externs));
